...
void App::OnActivated(IActivatedEventArgs const& e)
{
    ...
    if (e.Kind() == ActivationKind::Protocol)
    {
        ...
        if (protocolArgs)
        {
            ...
            if (0 != wcsstr(protocolString, L"ms-gamebaruiextension:"))
            {
    ...
    if (uiExtArgs)
    {
        ...
        {
            // Game Bar UI Extension activation work here
            auto rootFrame = Frame();
            rootFrame.NavigationFailed({ this, &App::OnNavigationFailed });
            Window::Current().Content(rootFrame);

            std::wstring appExtId{ uiExtArgs.AppExtensionId() };
            if (0 == appExtId.compare(L"Extension"))
            {
            ...
            {
                m_uiExtensionSettings = XboxGameBarUIExtension(
                    uiExtArgs,
                    Window::Current().CoreWindow(),
                    rootFrame);
        ...
        //rootFrame.Navigate(xaml_typename<UIExtSettingsSampleCPP::Extension>(), m_uiExtension1);
        Window::Current().Activate();
    }
}
